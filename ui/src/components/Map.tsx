...
const Map = ({ searchTerm, selectedCategories, onLocationSelect }: MapProps) => {
  const [selectedMarker, setSelectedMarker] = useState<number | null>(null);

  // Mock locations with reviews
  const locations = [
    { id: 1, lat: 40.7128, lng: -74.0060, name: "Central Park Pool", category: "pools", reviews: 23 },
    { id: 2, lat: 40.7589, lng: -73.9851, name: "Plaza Hotel", category: "hotels", reviews: 156 },
    { id: 3, lat: 40.7505, lng: -73.9934, name: "Times Square Cafe", category: "restaurants", reviews: 89 },
    { id: 4, lat: 40.7829, lng: -73.9654, name: "Riverside Park", category: "parks", reviews: 45 },
    { id: 5, lat: 40.7410, lng: -73.9897, name: "Chelsea Recreation Center", category: "recreation", reviews: 34 }
  ];
    ...
    <Card className="relative h-80 bg-gray-800 overflow-hidden border-gray-700 shadow-lg">
      ...
      <div className="relative z-10 p-6 h-full flex flex-col">
        <div className="text-center mb-4">
          <h2 className="text-lg font-semibold text-gray-100 mb-1">Location Reviews</h2>
          ...
          {filteredLocations.map((location, index) => (
            <div
              ...
              className={`absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-200 ${
                selectedMarker === location.id ? 'scale-125 z-20' : 'hover:scale-110 z-10'
              }`}
              style={{
                left: `${20 + (index * 15) % 60}%`,
                top: `${30 + (index * 20) % 40}%`
            ...
            >
              {/* Marker */}
              <div className={`w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-xs font-bold text-white ${
